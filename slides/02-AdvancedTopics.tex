...
\begin{frame}[fragile]{The use of packages}
	...
	\begin{itemize}
		...
		\item	Often used packages
			\begin{itemize}
			\item amsmath, graphicx, marvosym, microtype, booktabs, dcolumn,
				lipsum, fullpage, natbib, hyperref
		\end{itemize}
		\item	format:
\scriptsize
\begin{latexcode}
...
\begin{frame}[fragile]{The use of classes}
	\begin{itemize}
		\item	Typically one uses the \texttt{article} class---actually \texttt{scrartcl}
		\item	However, there is as well a \texttt{book}, \texttt{mininal},
		\texttt{report}, \texttt{beamer} class etcetera
		\item	Specific user written classes are \texttt{Koma-Script} and	\texttt{elsarticle} classes
...
\begin{frame}[fragile]{Exercise: fluff it up!}
	With a cool paper:
	\begin{enumerate}
		\item Use the \texttt{scrartcl} class with options: 11pt, abstracton, notitlepage
		\item Now add option parkskip (and perhaps twocolumn)
		\item One by one add the following packages and see what happens:
		\begin{enumerate}
			\item package fullpage
			\item package fourier
			\item package setspace with command thereafter \texttt{doublespace}
		\end{enumerate}
	\end{enumerate}
\end{frame}
...
\begin{frame}[fragile]{Exercise: cite or perish}
	Create a reference list by:
	\begin{enumerate}
		\item citing some people, note:
		\begin{latexcode}
\citet{refid} % in text citation
\citep[][]{refid} % between parentheses
		\end{latexcode}
	\item Indicate where the bibliography should be	
	\end{enumerate}
\end{frame}
...
\begin{frame}[fragile]{Exercise: insert cool picture}
\begin{enumerate}
	\item Insert the file \texttt{Powerphluff.jpg} and use the command
\begin{latexcode}
	width = 0.8\textwidth
\end{latexcode}
	to control the size
	\item Give the figure an appropriate caption (something perhaps with source)
\end{enumerate}
...
\begin{frame}[fragile]{Exercise: Create a lame table}
...
float.pos = "hb!")
	  ...
	  \begin{frame}[fragile]
	  \frametitle{Resulting table}
	  \input{Table.tex}
	\end{frame}



\begin{frame}[fragile]{Exercise: Be as lazy as possible!}
Now 
\begin{latexcode}
\input{Table.tex}
\end{latexcode}
...
\begin{frame}[fragile]{Exercise: Create slides or powerphluff!}
	\begin{block}{Create a couple of slides containing at least}
		\begin{itemize}
			\item Titlepage
			\item Introduction
			\item Conclusion
